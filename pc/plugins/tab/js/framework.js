...
$(function () {
    ...
    $("input:text[class=''],input:password[class=''],input:text[class*=validate],input:password[class*=validate]").each(function () {
        if ($(this).attr("keepDefaultStyle") == "true" || $(this).attr("keepDefaultStyle") == true) {
        } else {
...
jQuery.SelectBox = function (D, n) {
    ...
    function J(M) {
        if ($(M.target).attr("id") == g || $(M.target).attr("id") == u || $(M.target).parent().attr("class") == "selectbox-wrapper" || $(M.target).attr("class") == "selectbox-wrapper" || $(M.target).parents(".selectbox-wrapper").length > 0) {
        } else {
            a()
        }
    }
